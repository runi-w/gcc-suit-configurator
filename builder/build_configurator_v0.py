...
def soften_drape(L_img, mask=None):
    ...
    return im.filter(_IF.GaussianBlur(DRAPE_SOFT_PX * RSCALE))
...
for cid, fn, openf in CUTS:
    ...
    ud = datauri(soften_drape(_fit(L).convert("L"), _m), "JPEG", quality=86, optimize=True)
...
function buildDock(){renderTabs();renderPanel();
  ...
  // Front/Back: only front views are rendered, so Back stays disabled until back renders exist.
  const vf=document.getElementById('viewFront');if(vf)vf.onclick=()=>{};}
...
html = (HTML.replace("__CUTS__", json.dumps(cuts))
            ...
            .replace("__WARP_CAP__", f"{16 * RSCALE:.2f}"))
